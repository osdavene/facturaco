...
<div class="max-w-6xl mx-auto pb-36 lg:pb-0">
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 class="font-display font-bold text-2xl">Nueva Remisión</h1>
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                <div class="card p-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-slate-400 mb-1 uppercase tracking-wider">Documento</label>
                        ...
                        <div class="sm:col-span-2">
                            <label class="block text-xs font-semibold text-slate-400 mb-1 uppercase tracking-wider">Dirección de Entrega</label>
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b border-[#1e2d47]">
                                    ...
                                    <th class="text-center text-[10px] font-semibold text-slate-500 uppercase pb-2 px-2 w-16">Unidad</th>
                                    <th class="text-right text-[10px] font-semibold text-slate-500 uppercase pb-2 px-2 w-28">Precio Ref.</th>
            ...
            <div class="space-y-4">
                ...
                <div class="card p-5">
                    ...
                    <div class="flex justify-between items-center">
                        <span class="text-slate-400 text-sm">Total referencia</span>
                        <span id="display-total"
                              class="font-display font-bold text-xl text-amber-500">$0</span>
                    </div>
                    <p class="text-xs text-slate-600 mt-2">
                        * El precio es referencial. Los impuestos se aplican al convertir en factura.
                    </p>
                </div>
            ...
            <div class="px-4 py-2.5 hover:bg-[#141c2e] cursor-pointer
                ...
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <div class="text-xs text-slate-500">${p.codigo} · Stock: ${p.stock_actual}</div>
                    </div>
...
function updateItem(id, campo, valor) {
    const item = items.find(i => i.id === id);
    if (!item) return;
    item[campo] = ['cantidad','precio_unitario'].includes(campo) ? parseFloat(valor)||0 : valor;
    actualizarFilaTotales(id);
    calcularTotal();
}
...
function actualizarFilaTotales(id) {
    const item = items.find(i => i.id === id);
    if (!item) return;
    const tot  = item.cantidad * item.precio_unitario;
    ...
    tbody.innerHTML = items.map((item, idx) => `
        <tr class="border-b border-[#1e2d47]/30" data-id="${item.id}">
            ...
            <td class="py-2 px-2 w-16">
                <input type="text" name="items[${idx}][unidad]"
                       value="${item.unidad}"
                       onchange="updateItem(${item.id},'unidad',this.value)"
                       style="text-transform:uppercase;color:#e2e8f0"
                       class="w-full bg-transparent border-b border-[#1e2d47] text-sm
                              text-center py-1 focus:outline-none focus:border-amber-500 uppercase">
            </td>
            ...
            <td class="py-2 pl-2 text-right w-28">
                <span id="total-item-${item.id}" class="font-mono text-sm font-semibold"
                      style="color:#e2e8f0">${fmt(item.cantidad * item.precio_unitario)}</span>
            </td>
            <td class="py-2 pl-1">
