    ...
    <div class="sticky top-0 h-screen flex flex-col overflow-y-auto">
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            ...
            <a href="{{ route('backoffice.dian') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-colors
                      {{ request()->routeIs('backoffice.dian*') ? 'bg-amber-500/10 text-amber-500' : 'text-slate-500 hover:bg-[#1a2235] hover:text-slate-200' }}">
                <i class="fas fa-file-invoice-dollar w-4 text-center"></i>
                <span>Integración DIAN</span>
            </a>

            <a href="{{ route('backoffice.correo') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-colors
                      {{ request()->routeIs('backoffice.correo*') ? 'bg-amber-500/10 text-amber-500' : 'text-slate-500 hover:bg-[#1a2235] hover:text-slate-200' }}">
